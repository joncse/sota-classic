...
function addonEcho(msg)
	C_ChatInfo.SendAddonMessage(SOTA_MESSAGE_PREFIX, msg, "RAID");
end
...
--]]
function SOTA_UCFirst(playername)
...
--]]
function SOTA_GetRaidRank(nameAndRealm)	
...
function SOTA_RefreshGuildRoster()
	...
	for n=1,memberCount,1 do
		local nameAndRealm, rank, rankIndex, _, class, zone, publicnote, officernote, online = GetGuildRosterInfo(n)

		-- Note: [Name] comes with Realm; aka [Mimma-Pyrewood Village].
...
--]]
function SOTA_GetGuildPlayerInfo(nameAndRealm)
	local playername = SOTA_GetPlayerName(nameAndRealm);

	playername = SOTA_UCFirst(playername);

	for n=1, table.getn(GuildRosterTable), 1 do
		if GuildRosterTable[n][1] == playername then
...
--]]
function SOTA_GetRaidInfoForPlayer(nameAndRealm)
	local playername = SOTA_GetPlayerName(nameAndRealm);
...
function SOTA_Call_CheckClassDKP(playerclass, playername)
	...
	if playername then
		SOTA_whisper(playername, string.format("Top %d DKP for %ss:", MAX_CLASS_DKP_WHISPERED, playerclass));
		for n=1, table.getn(classtable), 1 do
			if n <= MAX_CLASS_DKP_WHISPERED then
				SOTA_whisper(playername, string.format("%d - %s: %d DKP", n, classtable[n][1], 1*(classtable[n][2])));
			end
		end
	else
...
--]]
function SOTA_GetDKP(nameAndRealm)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	local dkp = nil;
...
function SOTA_Call_SubtractPlayerDKPPercent(nameAndRealm, percent)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	if SOTA_IsInRaid(true) then
		RaidState = RAID_STATE_ENABLED;
		SOTA_RequestMaster();
...
function SOTA_SubtractPlayerDKPPercent(nameAndRealm, percent, silentmode)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	playername = SOTA_UCFirst(playername);
	local playerInfo = SOTA_GetGuildPlayerInfo(playername);
	if playerInfo then
...
--]]
function SOTA_Call_IncludePlayer(transactionID, nameAndRealm)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	if SOTA_IsInRaid(true) then
		RaidState = RAID_STATE_ENABLED;
...
function SOTA_IncludePlayer(transactionID, nameAndRealm, silentmode, skipApplyDkp)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	local transaction = SOTA_GetTransaction(transactionID);
	if not transaction then
...
--]]
function SOTA_Call_ExcludePlayer(transactionID, nameAndRealm)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	if SOTA_IsInRaid(true) then
		RaidState = RAID_STATE_ENABLED;
...
function SOTA_ExcludePlayer(transactionID, nameAndRealm, silentmode, skipApplyDkp)
	local playername = SOTA_GetPlayerName(nameAndRealm);
	local transaction = SOTA_GetTransaction(transactionID);
	if not transaction then
...
function SOTA_UpdateLocalDKP(nameAndRealm, dkpAdded)
	local receiver = SOTA_GetPlayerName(nameAndRealm);
...
function SOTA_ToggleIncludePlayerInTransaction(nameAndRealm)
	local playername = SOTA_GetPlayerName(nameAndRealm);
